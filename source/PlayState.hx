...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Ghost Tap Disabled', 0.2], //From 0% to 19%
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'heaven1': //madafakas trolface
				var bg:BGSprite = new BGSprite('heaven/skyBG1', -500, -300, 0.4, 0.4);
				bg.setGraphicSize(Std.int(bg.width * 4.9));
				bg.updateHitbox();
				add(bg);

				var bg2:BGSprite = new BGSprite('heaven/skyBG13', -50, 200, 0.9, 0.9);
				bg2.setGraphicSize(Std.int(bg2.width * 2.2));
				add(bg2);

			case 'heaven2': //fakin rayipo
				var bg:BGSprite = new BGSprite('heaven/skyBG2', -200, -100, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 5.9));
				bg.updateHitbox();
				add(bg);

				var bg2:BGSprite = new BGSprite('heaven/skyBG13', -50, 250, 0.9, 0.9);
				bg2.setGraphicSize(Std.int(bg2.width * 2.2));
				add(bg2);
				
				var nube:BGSprite;
					nube = new BGSprite('heaven/skyBG13', -50, 200, 0.9, 0.9);
				    nube.setGraphicSize(Std.int(nube.width * 2.2));
				    add(nube);
					nube.visible = true;
				    nube.screenCenter(X);
		            nube.scrollFactor.set();
				    nube.cameras = [camHUD];
					
			case 'heaven3': //RASU DASU
				var bg:BGSprite = new BGSprite('heaven/spaceBG', -300, -460, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 4.5));
				bg.updateHitbox();
				add(bg);

				var bg2:BGSprite = new BGSprite('heaven/skyBG13', -150, 150, 0.9, 0.9);
				bg2.setGraphicSize(Std.int(bg2.width * 2.9));
				add(bg2);
				
				var nube:BGSprite;
					nube = new BGSprite('heaven/starsBG', 0, 0, 0.9, 0.9);
				    add(nube);
					nube.visible = true;
				    nube.screenCenter(X);
		            nube.scrollFactor.set();
			...
			case 'heaven4': //eeee bauker y eli creo
				var bg:BGSprite = new BGSprite('heaven/skyBG3', -7000, -4300, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 7.9));
				bg.updateHitbox();
				add(bg);

				var bg2:BGSprite = new BGSprite('heaven/skyBG31', -150, 630, 0.9, 0.9);
				bg2.setGraphicSize(Std.int(bg2.width * 2));
				add(bg2);
				
				var bg3:BGSprite = new BGSprite('heaven/skyBG31', -950, -150, 0.7, 0.7);
				bg3.setGraphicSize(Std.int(bg3.width * 1.4));
				add(bg3);
				
				var bg4:BGSprite = new BGSprite('heaven/skyBG31', 350, -550, 0.8, 0.8);
				bg4.setGraphicSize(Std.int(bg4.width * 1.2));
				add(bg4);
				
				var bg5:BGSprite = new BGSprite('heaven/skyBG31', 2050, -650, 0.8, 0.8);
				bg5.setGraphicSize(Std.int(bg5.width * 1.4));
				add(bg5);
				
				var bg6:BGSprite = new BGSprite('heaven/skyBG31', -2450, 50, 0.5, 0.5);
				bg6.setGraphicSize(Std.int(bg6.width * 1.1));
				add(bg6);
				
				var nube:BGSprite;
					nube = new BGSprite('heaven/skyBG31', 0, 500, 0.9, 0.9);
					nube.setGraphicSize(Std.int(nube.width * 1.5));
				    add(nube);
					nube.visible = true;
				    nube.screenCenter(X);
		            nube.scrollFactor.set();
			...
			case 'heaven5': //ALE VS LUCHO TWTFFFFFF
				var bg:BGSprite = new BGSprite('heaven/skyBG4', -200, -560, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 3.5));
				bg.updateHitbox();
				add(bg);

				var bg2:BGSprite = new BGSprite('heaven/skyBG42', -50, 250, 0.9, 0.9);
				bg2.setGraphicSize(Std.int(bg2.width * 2.2));
				add(bg2);
				
				var nube:BGSprite;
					nube = new BGSprite('heaven/skyBG13', -50, 140, 0.9, 0.9);
				    nube.setGraphicSize(Std.int(nube.width * 2.2));
				    add(nube);
					nube.visible = true;
				    nube.screenCenter(X);
			...
			case 'heaven6': //lucho gigante ctm lucho gigante
				
				var bg:BGSprite = new BGSprite('heaven/skyBG5', -900, -600, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 4.7));
				bg.updateHitbox();
				add(bg);
		...
		switch (curStage)
		{
			case 'heaven6': //lucho gigante ctm lucho gigante
				...
				var bg2:BGSprite = new BGSprite('heaven/skyBG53', -500, 250, 0.9, 0.9);
				bg2.setGraphicSize(Std.int(bg2.width * 3.2));
				add(bg2);
		}
		...
		FlxG.camera.zoom = defaultCamZoom; //tptp
		...
		scoreTxt2 = new FlxText(0, healthBarBG.y + 66, FlxG.width, "", 24);
		scoreTxt2.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(tmr.loopsLeft % 2 == 0) {
					if (boyfriend.animation.curAnim != null && !boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.stunned)
					{
						boyfriend.dance();
					}
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingString == '?') {
			...
			if (PlayState.SONG.song.toLowerCase() == "the-end")
			{
				scoreTxt2.text = 'puedes bajar la dificultad con "ENTER"';
			}
		} else {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'RIO': //TELEPORT
				...
				var dia:FlxSprite;
				dia = new FlxSprite(0, 0);
				dia.frames = Paths.getSparrowAtlas('heaven/pokeballs');
				add(dia);
				dia.animation.addByPrefix('open', 'pokefall', 30, true);
				dia.visible = false;
				dia.screenCenter(X);
			...
			case 'Blammed Lights':
				...
				if (lightId > 0 && curLightEvent != lightId) {
					...
					if (PlayState.SONG.song.toLowerCase() == "lucho")
					{
						nube = new FlxSprite(-50, 200);
						//nube = new BGSprite('heaven/skyBG13', -50, 200, 0.9, 0.9); //teleport22
						nube.frames = Paths.getSparrowAtlas('skyBGanim2');
						nube.setGraphicSize(Std.int(nube.width * 2.2));
						add(nube);
						nube.visible = false;
						nube.screenCenter(X);
						...
						nube.y = -750;
						nube.flipX = true;
						nube.animation.addByPrefix('moveCloud', 'cloud', 20, true);
						nube.animation.addByPrefix('eraseCloud', 'dissapear', 20, false);
						if (ClientPrefs.downScroll){
							nube.y = 85;
						}
					}
					else
					{
						nube = new FlxSprite(-50, 200);
						//nube = new BGSprite('heaven/skyBG13', -50, 200, 0.9, 0.9); //teleport22
						nube.frames = Paths.getSparrowAtlas('skyBGanim');
						nube.setGraphicSize(Std.int(nube.width * 2.2));
						add(nube);
						nube.visible = false;
						nube.screenCenter(X);
					...
					switch(lightId) {
						case 1: //Blue + clouds
							nube.visible = true;
							nube.animation.play('moveCloud');
							color = 0xfCFFFFF;
						case 2: //stop clouds
							nube.animation.play('eraseCloud');
						case 3: //Pink
							color = 0xfff794f7;
						case 4: //Red + clouds
							nube.visible = true;
							nube.animation.play('moveCloud', true);
							color = 0xfff96d63;
						case 5: //PUT AT THE START
							nube.visible = false;
							nube.animation.play('eraseCloud');
					}
					...
					case 'Hurt Note': //Hurt note
						...
						if(!boyfriend.stunned)
						{
							...
							if(!endingSong)
							{
								...
								else health -= 0.7; //0.06 + 0.04 = -0.1 (-5%) of HP if you hit a hurt sustain note
