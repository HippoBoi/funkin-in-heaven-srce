...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var blackBarThingie:FlxSprite = new FlxSprite(150, 100).loadGraphic(Paths.image('skyBG2'));
		blackBarThingie.setGraphicSize(Std.int(blackBarThingie.width * 1.3));
		...
		var bgYellow:FlxSprite = new FlxSprite(0, -1456).makeGraphic(FlxG.width, 386, 0xFFF9CF51);
		bgSprite = new FlxSprite(0, -156);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			weekThing.y += ((weekThing.height - 1660) * i + 20);
			weekThing.targetY = i;
		...
		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y - 150, 0, "", 32);
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		if(assetName == null || assetName.length < 1) {
			...
			bgSprite.animation.addByPrefix('bump', 'cabros', 24, false);
			bgSprite.animation.play('bump');
