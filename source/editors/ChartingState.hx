...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['RIO', "Value 1: JUST PUT 1 IT SHOULD ACTIVATE EVERYTHING"]
	];
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			{
				autosaveSong();
				EditorPlayState.daMania = _song.mania;
	...
	private function addNote():Void
	{
		...
		{
			_song.notes[curSection].sectionNotes.push([noteStrum, (noteData + Main.ammo[_song.mania]) % (Main.ammo[_song.mania] * 2), noteSus, noteTypeIntMap.get(daType)]);
		}
